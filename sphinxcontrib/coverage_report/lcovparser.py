...
Note: ``BRDA`` per-branch records are parsed for documentation purposes but
branch totals come from the ``BRF``/``BRH`` summary lines.
"""
import os
...
class LcovParser:
    """Parse an lcov ``.info`` coverage report into a plain dict structure."""
    ...
    def __init__(self, lcov_path, encoding="utf-8"):
        ...
        Raises:
            CoverageReportFileNotFound: If *lcov_path* does not exist.
        """
    ...
    def parse(self):
        """Parse the lcov file and return a coverage report dict.

        Returns:
            A dict with top-level coverage metrics and a ``packages`` list,
            mirroring the structure produced by :class:`CoberturaParser`.

        Raises:
            CoverageReportFileInvalid: If a line cannot be parsed (e.g. a
                numeric field contains non-numeric text).
        """
        ...
        with open(self.lcov_path, encoding=self.encoding) as fh:
            ...
            try:
                ...
                raise CoverageReportFileInvalid(
                    f"Cannot parse {self.lcov_path}: {exc}"
                ) from exc
    ...
    @staticmethod
    def _empty_module():
        """Return a blank module dict used as accumulator while parsing a record."""
        return {
            "name": "", "filename": "",
    ...
    def _finalize_module(m):
        """Compute derived fields (rates, missed lines, functions) in-place."""
        da = m.pop("_da")
